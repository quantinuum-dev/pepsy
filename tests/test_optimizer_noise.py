...
def test_coalesced_pauli_stream_replays_with_cuda_torch_gates():
    """Coalesced replay must not send an already-CUDA gate through NumPy."""
    torch = pytest.importorskip("torch")
    ...
    state.apply_to_arrays(
        lambda array: torch.as_tensor(
            array, dtype=torch.complex128, device="cuda:0"
        )
    )
    gate = torch.tensor(
        np.array([[1.0, 1.0], [1.0, -1.0]], dtype=complex) / np.sqrt(2.0),
    ...
    assert result.leaves[0].optimizer.p[0].data.device.type == "cuda"
...
def test_noisy_shot_auto_strategy_coalesces_only_rare_fault_streams():
    """The conservative auto policy preserves the legacy result by default."""
    factory = lambda: pepsy.MpsOptimizer(
        qtn.MPS_computational_state("0"), chi=4, mode="mpo"
    )
    rare = pepsy.run_noisy_shots(
        factory,
        [(qu.hadamard(), 0)],
        pepsy.PauliErrorModel.depolarizing(1.0e-3),
        shots=64,
        seed=4,
        run_kwargs={"progbar": False},
        strategy="auto",
    )
    frequent = pepsy.run_noisy_shots(
        factory,
        [(qu.hadamard(), 0)],
        pepsy.PauliErrorModel.bit_flip(1.0),
        shots=3,
    ...
    assert isinstance(rare, pepsy.CoalescedTrajectoryResult)
    assert rare.shots == 64
    assert isinstance(frequent, pepsy.NoisyShotResult)
    assert frequent.shots == 3


def test_noisy_shot_auto_strategy_uses_an_exact_branch_cap_fallback():
    """A cap fallback replays independent samples instead of truncating leaves."""
    factory = lambda: pepsy.MpsOptimizer(
        qtn.MPS_computational_state("0"), chi=4, mode="mpo"
    )
    kwargs = dict(
        ...
        gates=[(qu.hadamard(), 0)],
        error_model=pepsy.PauliErrorModel.depolarizing(1.0),
        shots=128,
        seed=4,
        run_kwargs={"progbar": False},
        max_branches=1,
    ...
    with pytest.raises(RuntimeError, match="branch cap"):
        pepsy.run_coalesced_noisy_shots(**kwargs)
    fallback = pepsy.run_noisy_shots(
        **kwargs,
        strategy="auto",
        auto_max_expected_faults=1.0,
    )

    assert isinstance(fallback, pepsy.NoisyShotResult)
    assert fallback.shots == 128


def test_noisy_shot_auto_strategy_rejects_unforced_controls():
    """Measurement branching is not mistaken for a rare-noise win."""
    result = pepsy.run_noisy_shots(
        lambda: pepsy.MpsOptimizer(
            qtn.MPS_computational_state("0"), chi=4, mode="mpo"
        ),
        [(qu.hadamard(), 0), ("measure", "Z", 0)],
        pepsy.PauliErrorModel(),
        shots=4,
        seed=4,
        run_kwargs={"progbar": False},
    ...
    assert isinstance(result, pepsy.NoisyShotResult)
