...
The public family facade is `pepsy.operators.mpo_higher_order`. Spatial MPO
clusters and ordered multi-factor products are documented separately in
[`mpo_cluster.md`](mpo_cluster.md); they are not another meaning of this
paper's history `order`.
...
## Spatial MPO clusters are a separate family

For a spatial cluster cutoff, use the dedicated
[`MPO cluster API`](mpo_cluster.md). Its `cluster_size` is not the Taylor or
history `order` documented on this page. The cluster API also owns the joint
ordered construction of `exp(A) @ exp(B) @ exp(C)`, formed locally before one
MPO topology is assembled; it is not sequential multiplication of separately
truncated full-lattice MPO layers.
