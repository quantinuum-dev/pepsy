...
For the direct modes, `fit_layer_mode="joint"` (default) compresses all
tagged layers in a boundary target together. Set
`fit_layer_mode="sequential"` to compress layers one at a time in the order
specified by `layer_tags`; this supports explicitly tagged multilayer targets
such as `("BRA", "PEPO", "KET")`. FIT modes do not support sequential layer
compression.
