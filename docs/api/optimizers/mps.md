...
control adaptive stopping from FIT's final local-norm change. The default
`fit_rtol=1e-8` compares the retained local Frobenius norm between sweeps. It
is a cheap convergence proxy, not a direct overlap or literal infidelity;
`fit_rtol="auto"` remains available for dtype-aware tolerances of
`1e-3`, `1e-5`, or `1e-8` for 16-, 32-/complex64-, or higher-precision data.
Pass `fit_rtol=None` for fixed iterations. The old
