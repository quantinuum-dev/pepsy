...
The compatibility wrappers `reg_complex_svd_torch()` and
`reg_complex_svd_jax()` are also available directly from `pepsy`, e.g.
`import pepsy as py; py.reg_complex_svd_torch()`.
