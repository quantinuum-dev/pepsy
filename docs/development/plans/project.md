...
Status: historical workstreams and proposals; not a current release checklist
Workstream snapshot: 2026-09-14; status clarification: 2026-09-25
...
This snapshot was written for version `0.4.1`. The package has since reached
`0.5.0`; use the [changelog](https://github.com/quantinuum-dev/pepsy/blob/develop/CHANGELOG.md) and
[migration guide](../api-migration.md) for released changes, and
`pyproject.toml` for current version and dependency requirements. Completed
work, prototypes, and deferred proposals below retain their dated context.
...
- Concept note: `../notes/belief_propagation.md`.
...
`tests/test_bp_relay.py`: on a 3×3 `D=2` PEPS the error falls 22 % (BP) →
...
- Concept note: `../notes/belief_propagation.md` (loop / cluster expansion sections).
...
- **Audit `quimb.tensor` BP + gauging API** and prefer wrapping it over
  ...
  in `../notes/quimb.md`. (The sampling subpackage already wraps quimb's
...
- Concept note: `../notes/symmray.md`.
- Fermionic MPO convention note: `../notes/fermionic_mpo.md`.
...
- Package tests: `tests/test_vmc_api.py` (gated on optional deps).
...
- Package: `pytest -q -o addopts='' tests/test_vmc_api.py tests/test_public_api.py`
  (VMC deps guarded with `pytest.importorskip`).
- VMC API: `pytest -q -o addopts='' tests/test_vmc_api.py` plus ED checks for tiny
...
- Concept note: `../notes/symmray.md` (fermionic amplitudes).
- Gao et al., *Fermionic tensor network contraction for arbitrary geometries*,
  Phys. Rev. Research 7, 023193 (2025), DOI `10.1103/PhysRevResearch.7.023193`.
...
- Do not reintroduce removed flat modules (`pepsy.core`, `pepsy.gates`,
  ...
  guards this.
- Promote finalized, user-facing designs from `../notes/` into `docs/`
  (tutorials / how-to).
...
- `pepsy_examples/` is the downstream validation surface; keep its notebooks
  on current public namespaces and consult that repository's `PLAN.md`.
