"""Finite one-dimensional MPO cluster-basis expansions.

This module implements the one-dimensional construction of Vanhecke,
Vanderstraeten, and Verstraete, arXiv:1912.10512.  A finite connected
interval is exponentiated locally, its already represented disconnected
pieces are subtracted, and the connected residual is put into an MPO path.
The paths are allowed to be concatenated only when their intervals are
disjoint, which is the size-extensive cluster expansion.
...
class CompiledMPOClusterExp:
    ...
    def __init__(self, basis):
        if not isinstance(basis, MPOClusterBasisExpansion):
            raise TypeError("basis must be an MPOClusterBasisExpansion.")
        self.basis = basis
...
class MPOClusterBasisExpansion:
    ...
    def _normalize_factor(factor):
        ...
        if isinstance(factor, MPOClusterBasisExpansion):
            raise TypeError("nested MPOClusterBasisExpansion factors are not supported.")
    ...
    def compile_exp(self):
        ...
        return CompiledMPOClusterExp(self)
